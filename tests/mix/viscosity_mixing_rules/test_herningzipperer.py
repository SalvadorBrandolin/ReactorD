...
def test_herning_zipperer():
    ...
    gas = rd.mix.IdealGas([a, b, c], viscosity_mixing_rule="herning_zipperer")
    solution = rd.mix.IdealSolution(
        [a, b, c], viscosity_mixing_rule="herning_zipperer"
    )
    ...
    numerator = (
        (np.multiply(mole_fractions, pure_v).T * np.array([4, 3, 6])).T
    ).sum(axis=0)

    denominator = ((mole_fractions.T * np.array([4, 3, 6])).T).sum(axis=0)

    mix_v_by_hand = numerator / denominator
