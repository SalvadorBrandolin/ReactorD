...
def grunberg_nissan(
    ...
    r"""Calculate the mixture's viscosity with the Grunberg-Nissan mixing rule.
    ...
    Ref: https://www.nature.com/articles/164799b0

    .. math::
           ln({\mu_{mix}}_{(T,P)}) = \sum z_i ln({\mu_i}_{(T,P)})
    ...
    if mixture.phase_nature == "liquid":
        ...
        log_pure_viscosities = np.log(pure_viscosities)
    ...
    # Mixing rule
    mixture_viscosities = np.exp(
        np.multiply(log_pure_viscosities, molar_fractions).sum(axis=0)
    )
